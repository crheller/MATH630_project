...
First, load the data. Here, we simply load the data that we've previously tidied. In order to see how this was done (in its final form) please see the code contained in the file "P3_EDA.Rmd". This code tidies the data and caches it locally into our data folder, from which we load it here for the exploratory data analysis
...
In the exploratory data analysis above, we re-made all of the figures from the paper (except one) and listed some summary stastics that were reported by the authors as well. We left the more "rigourous" statistical tests for this section (and the last plot which is more directly related to some of these tests), and will address them here. We will also replot each figure as necessary to make it easier to follow along.

We noted, when reading the paper, that the authors depended heavily on the use of ttests, which are paramteric tests that assume normality in the data. However, the sample size of the data in this paper was quite small making it difficult to determine if their data did in fact come from a normal distribution. In order to validate their results, we will include, at each point a ttest is used, a non-parametric alternative (namely, a Wilcoxon Ranksum test or a shuffle test, where apprpriate) in order to either corrabate or condtradict their reported signficance finding.
...
Nice, we see that we've repicated the author's results, as well as added an **extension** plot to nicely illustrate that for every single subject we see an increase in time spent attenting to the environment in the deocrated classroom. This suports the author's conclusions that their results were not simply due to a couple of outlier students.

Next the authors compare time off task faceted by subtype of distraction. They find the folliwng: "In the sparse-classroom condition, the children
...
Non-parametric is consitent with parametric test.
...
First the author's investigate the relationship between total time spent off task and post test scores as a measure of whether the two variables are related. They find: "Children who spent more time off task tended to have
